...
```
Reported → Triaged → In Progress → In Review → Resolved → Verified
```
...
1. Send a direct Messenger message to the developer
2. Re-tag the issue with `escalated` in GitHub
...
| [`docs/deployment-plan.md`](./deployment-plan.md) | Deployment and rollback procedures |
| [`docs/risk-register.md`](./risk-register.md) | Identified project risks and mitigations |